...
In the early implementation of `serene/apps/web/src/server/sources/libre.ts`, we _re-derived_ the headers and auth flow from RFC + community references rather than porting from luff. This produced three speculative deviations:
...
- Header set must match luff exactly unless we have _evidence_ a deviation is needed.
